...
class Database {
    ...
    public function getTotalCustomersCount() {
        $this->query("SELECT COUNT(*) as count FROM users WHERE user_type = 'customer' AND status = 'active'");
        $result = $this->single();
        return $result['count'] ?? 0;
    }

    public function getTotalTailorsCount() {
        $this->query("SELECT COUNT(*) as count FROM users WHERE user_type = 'tailor' AND status = 'active'");
        $result = $this->single();
        return $result['count'] ?? 0;
    }

    public function getTotalRevenue() {
        $this->query("SELECT SUM(total_amount) as revenue FROM orders WHERE payment_status = 'paid'");
        $result = $this->single();
        return $result['revenue'] ?? 0;
    }

    public function getPendingOrdersCount() {
        $this->query("SELECT COUNT(*) as count FROM orders WHERE status = 'pending'");
        $result = $this->single();
        return $result['count'] ?? 0;
    }
